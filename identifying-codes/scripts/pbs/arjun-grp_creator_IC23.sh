...
#  call with: qsub arjun-grp_mpi_IC23.pbs
#  
################################################################################
...
GIS_DIR="${PROJECT_DIR}/instances/gis"
SOFTWARE_DIR="${PROJECT_DIR}/software"
SCRIPTS_DIR="${PROJECT_DIR}/scripts"

RESULT_DIR="${PROJECT_DIR}/results/${EXPID}-gis"
OUTPUT="out-ic"
TMP_INFILE_DIR="in"
...
# )
# k_array=( "1" "2")
# RESULT_DIR="${PROJECT_DIR}/results-test/${EXPID}-gis"
# tlimit="30"
# arjun_tlimit="30"
################
# end testing
...
for k in ${k_array[@]}; do
    for subdir in ${networks_arr[@]}; do
        for gis_file in ${GIS_DIR}/k${k}/${subdir}/*; do
            if [[ $gis_file == *.gz ]]; then
                files_arr+=( ${gis_file} )
            fi
        done
...
# Create todo
for configuration in "${config_arr[@]}"; do
    echo "configuration ${configuration}"
    fin_out_dir="${OUTPUT}/config-${at_config}"
    ...
    for file in ${files_arr[@]}; do
        ...
        tmp_fin_dir="${TMP_INFILE_DIR}/k${k}/${networktype}"
        ...
        echo "mkdir -p ${OUTPUT}/config-${at_config}/k${k}/${networktype}" >> todo
        # create dir in project directory to permanently store script output files and timeout files
        echo "mkdir -p ${RESULT_DIR}/config-${at_config}/k${k}/${networktype}/" >> todo
        ...
        baseout="${OUTPUT}/config-${at_config}/k${k}/${networktype}/${basefilename}"
        
        # Specify the command with which we will call the solver
        command="/usr/bin/time --verbose -o ${baseout}.timeout.out timeout ${arjun_tlimit} ./arjun-grp --fastbackw=0 --simp=0 --groupind=1 --backward=1 --forward=0 --verb=0 ${configuration} ${tmp_fin_dir}/${basefilename} "
        ...
        echo "echo \"c Configuration:      ${at_config}\" >> ${baseout}.solver.out" >> todo
        ...
        echo "mv ${baseout}.solver.out* ${RESULT_DIR}/config-${at_config}/k${k}/${networktype}/" >> todo
        echo "mv ${baseout}.timeout* ${RESULT_DIR}/config-${at_config}/k${k}/${networktype}/"  >> todo
        linesperjob=$((linesperjob+2))
        
        numjobs=$((numjobs+1))
    done
    let at_config=at_config+1
done
