...
#  call with: qsub encoding_is_mpi_IC24.pbs
#
################################################################################
...
#     "--remove_supersets --check_2_neighbourhood"
)
...
cp -R ${SOFTWARE_DIR}/cplex/python/3.5/x86-64_linux .
...
touch todo
mkdir -p ${OUTPUT}      
mkdir -p ${TMP_ENC_DIR} # TODO: in the following, ensure that the files containing the encoding are properly created, stored and moved 
...
for cnfg in "${config_arr[@]}"; do
    ...
    for file in ${files_arr[@]}; do
        ...
        for k in ${k_array[@]}; do
            ...
            echo "echo \"c ILP configuration:  config${at_cnfg}\" >> ${baseout}.encoder.out" >> todo
            ...
            echo "echo \"c \" >> ${baseout}.encoder.out" >> todo
            # ----------------------------------------------------------------------
            mylinesperjob=$((mylinesperjob+28))
